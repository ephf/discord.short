...
for(key of keys) {
    if(key == args[0]) {
        found = true;
        if(!args[1]) out.error('You need to specify the setting you wish to change');
        if(args[1] == 'run') {
            execSync(`node ${key}`, {stdio: [process.stdin, process.stdout, process.stderr]});
        } else if(args[1] == 'add') {
            execSync(`ds addbot ${key}`, {stdio: [process.stdin, process.stdout, process.stderr]});
        } else if(args[1] == 'remove') {
            execSync(`ds rembot ${key}`, {stdio: [process.stdin, process.stdout, process.stderr]});
        } else {
