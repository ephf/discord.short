...
class ShortClient {
  ...
  reactEvent(message, reaction, callback) {
    message.react(
      /<:[\w~]+:[0123456789]+>/.test(reaction)
        ? /<:[\w~]+:([0123456789]+)>/.exec(reaction)[1]
        : reaction
    );
    this._reactEvents.push({
      message,
      reaction: /<:[\w~]+:[0123456789]+>/.test(reaction)
        ? /<:([\w~]+):/.exec(reaction)[1]
        : reaction,
      callback,
    });
  ...
  async setUserData(data, id) {
    ...
    if (typeof data != "object")
      error('the "data" param in setUserData needs to be an object');
    if (
      !(await userdat.findOne({ _id: id ? id : this.data.config.author.id }))
    ) {
      await new userdat({
    ...
    await userdat.findOneAndUpdate(
      { _id: id ? id : this.data.config.author.id },
      {
        _id: id ? id : this.data.config.author.id,
        data: override,
  ...
  async setGuildData(data, id) {
    ...
    if (typeof data != "object")
      error('the "data" param in setGuildData needs to be an object');
    if (
      !(await serverdat.findOne({ _id: id ? id : this.data.config.guild.id }))
    ) {
      await new serverdat({
    ...
    await serverdat.findOneAndUpdate(
      { _id: id ? id : this.data.config.guild.id },
      {
        _id: id ? id : this.data.config.guild.id,
        data: override,
  ...
  async getGuildData(id) {
    let serverdat = schemas.server;
    let dat = await serverdat.findOne({
      _id: id ? id : this.data.config.guild.id,
    });
    if (dat && !dat.data) dat.data = {};
  ...
  async login(config) {
    ...
    this.bot.on("messageReactionAdd", (reaction, user) => {
      this._reactEvents.forEach((react) => {
        if (
          react.reaction == reaction.emoji.name &&
          reaction.message.id == react.message.id &&
    ...
    if (config.mongo && this.settings.mongoConnect) {
      await require("./mongodb/mongo")(config).then(async (item) => {
        ...
        if (typeof this.data.events["MONGOCONNECT"] == "function")
          this.data.events["MONGOCONNECT"]();
      });
    } else {
      this.data.connected = true;
    }
